...
%% -spec append :: [a] ::: c [b] ::: c -> [a, b] ::: c
%%       append :: ListA ListB -> ListA ++ List B
...
difference({List_a,List_b})->
	{lists:filter(fun(A) -> not lists:member(A, List_b) end, List_a), []}.

%% - spec difference :: [a] ::: c [b] ::: c ->
%%		spec difference_helper :: member :: d -> member :: d, b 
...
merge_helper(List_a,List_b) ->
	[A|Rest_a] = List_a,
	[B|Rest_b] = List_b,
	[A] ++ [B] ++ merge_helper(Rest_a, Rest_b).

%%	merge({[a,b,c], [d,e,f]}) -> {[a,d,b,e,c,f], []}

%% - spec merge :: [a] ::: c [b] ::: c ->
%%		  merge :: list_A -> [h|t] ::: list_B -> [h|t] ::: list_C
%% 		  merge :: t -> [h|t]
...
%% id() === append(id()) === difference(id()) === merge(id())

	%%{fun append/1, fun difference/1, fun merge/1}.

%% -spec id :: -> {a ::: function, b ::: function, c ::: function}
%%       id :: -> {append, difference, merge}
